...
import reimburse.cuc.com.bean.CostStatusInfo;
...
public class PieChartActivity extends Activity {
    ...
    private static final int SHOW_ALL_COST_SUM = 01;
    private Typeface mTf;//声明字体库
    private List<TongyiXiaofeiBean> tongyiXiaofeiBeanList = new ArrayList<>();
    private List<CostStatusInfo> costStatusInfoList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* //获取数据
        ...
        getTotalCostSumByUserId(user_id);
        //testPost();

    }


    public void getTotalCostSumByUserId(final Integer user_id) {

        new Thread() {
            public void run() {
                try {
                    // 1.指定提交数据的路径,post的方式不需要组拼任何的参数
                    String path = Constants.ANDROID_GET_TOTAL_COST_BY_USER_ID;
                    ...
                    String data = "user_id=" + URLEncoder.encode(String.valueOf(user_id));
                    ...
                    if (code == 200) {
                        ...
                        costStatusInfoList = JSON.parseArray(result, CostStatusInfo.class);

                        Message message = Message.obtain();
                        message.obj = costStatusInfoList;
                        message.what = SHOW_ALL_COST_SUM;
                        handler.sendMessage(message);
                        Log.e("网络请求，从服务端得到的报销数据", costStatusInfoList.size() + "");
   ...
   private void showToastInAnyThread(final String result) {
    ...
    private void generatePieData() {
        ...
        for (int i = 0; i < costStatusInfoList.size(); i++) {
            CostStatusInfo bean = costStatusInfoList.get(i);
            String type = bean.getBudget_type();
            float jine = Float.parseFloat(bean.getBudget_paper_passed_amount());
            entries.add(new Entry(jine, i));
            q.add(type);
        }
